...
public class GameEngineImpl implements GameEngine {
	...
	public void dealPlayer(Player player, int delay) {
		...
		if (player.getBet() > minBet) {
			...
			} while (playerResult + card.getScore() <= BUST_LEVEL);//When the result + next card value over 21, break
	...
	//Refill the card when the cards are ran out
	private void refillCards() {
		deckCard= this.getShuffledDeck();
	}
